...
- **🏟️ 多人大亂鬥** — 最多 16 人同場、即時看到彼此,限時(3／5／7 分鐘)搶 50 顆會重生的氣球,老師按鈕全場一起倒數起跑、即時排行。
- **🧑‍🏫 老師即時後台** — 學生上線狀態、目前關卡、完成時間**即時排行**;可一鍵廣播「載入關卡／重置／開始比賽」、開大亂鬥。同名自動視為重連,名單不會堆積幽靈。
...
![大亂鬥](docs/images/readme-arena.png)

最多 **16 人同場**、即時看到彼此的無人機,在共享戰場搶 **50 顆會重生的氣球**:

- 學生點畫面右下角 **🏟️ 大亂鬥** 進場 → 老師後台按 **▶ 3／5／7 分鐘** → 全場一起 3-2-1 倒數起跑。
- 撞破氣球 +1 分、2.5 秒後在別處重生;右上角**即時排行**,時間到顯示冠軍。
- **伺服器權威**:同一顆氣球只算最先撞到的人,大家看到的氣球與分數完全同步。
- **可切背景**:HUD 的「🌆 場景」鈕可在**格線競技場 ⇄ playground 場景**間切換(純視覺背景,不影響玩法)。
- 玩家之間互不碰撞,單純比誰搶得多。
