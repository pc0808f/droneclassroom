...
const LEVEL_STARTERS = {
    ...
    '1-3': '<xml xmlns="https://developers.google.com/blockly/xml"><block type="cf_takeoff" x="50" y="50"><value name="HEIGHT"><block type="math_number"><field name="NUM">2</field></block></value><next><block type="cf_rotate_cw"><value name="ANGLE"><block type="math_number"><field name="NUM">90</field></block></value><next><block type="cf_rotate_cw"><value name="ANGLE"><block type="math_number"><field name="NUM">90</field></block></value><next><block type="cf_rotate_cw"><value name="ANGLE"><block type="math_number"><field name="NUM">90</field></block></value><next><block type="cf_rotate_cw"><value name="ANGLE"><block type="math_number"><field name="NUM">90</field></block></value><next><block type="cf_land"></block></next></block></next></block></next></block></next></block></xml>',
    '1-4': '<xml xmlns="https://developers.google.com/blockly/xml"><block type="cf_takeoff" x="50" y="50"><value name="HEIGHT"><block type="math_number"><field name="NUM">3</field></block></value><next><block type="cf_forward"><value name="DIST"><block type="math_number"><field name="NUM">5</field></block></value><next><block type="cf_forward"><value name="DIST"><block type="math_number"><field name="NUM">5</field></block></value><next><block type="cf_forward"><value name="DIST"><block type="math_number"><field name="NUM">5</field></block></value><next><block type="cf_backward"><value name="DIST"><block type="math_number"><field name="NUM">15</field></block></value><next><block type="cf_land"></block></next></block></next></block></next></block></next></block></xml>',
...
function resolveBVHCollision(bvh, radius) {
    if (!bvh) return;
    _bvhSphere.center.copy(droneState.position);
    _bvhSphere.radius = radius || DRONE_RADIUS;
    let hit = false;
    bvh.shapecast({
...
function loadLevel(levelId) {
    ...
    // v1.4 T-104：載入對應關卡的 Blockly starter（不影響計時/drone 位置）
    loadLevelStarter(levelId);
...
function runProgram(workspace) {
    ...
    programPromise.then(() => {
        const allRings = programState.totalRings > 0 && programState.ringsCollected >= programState.totalRings;
        const allZones = currentLevel && currentLevel.passZones &&
                         passZoneProgress.length === currentLevel.passZones.length &&
                         passZoneProgress.every(p => p);
        // T-104: 程式結束時若達成過關條件，也上報老師後台
        if (allRings || allZones) {
            const elapsed = Date.now() - levelStartTime;
            const msg = allRings
                ? `🎉 完成！穿過 ${programState.totalRings} 個圈，用時 ${(elapsed/1000).toFixed(1)}s`
                : `🎉 完成！過關用時 ${(elapsed/1000).toFixed(1)}s`;
...
function clearSoccerField() {
    ...
    _soccerBVH = null;
    if (SOCCER.ball && SOCCER.ball.parent) SOCCER.ball.parent.remove(SOCCER.ball);  // 移除球形外框
}
...
function clampSoccerBounds() {
    const p = droneState.position, v = droneState.velocity, m = SOCCER_BALL_R;  // 用球框半徑當邊界內縮,球框不穿牆
